...
### Как временно выключить сайт

Самый удобный вариант для этого проекта — не удалять деплой, а включать режим техработ.

На Render:

1. Откройте сервис.
2. Зайдите в `Environment`.
3. Добавьте или измените `MAINTENANCE_MODE=true`.
4. При желании задайте свои тексты через `MAINTENANCE_TITLE` и `MAINTENANCE_MESSAGE`.
5. Сохраните изменения и дождитесь перезапуска сервиса.

Чтобы включить сайт обратно, просто:

1. удалите `MAINTENANCE_MODE`, или
2. поставьте `MAINTENANCE_MODE=false`.

В этом режиме сайт отвечает страницей техработ, а API возвращает `503 maintenance_mode`.
